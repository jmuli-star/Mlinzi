...
def load_transactions() -> list[dict]:
    ...
    df["amount_kes"] = pandas.to_numeric(
        df["amount_kes"].astype(str).str.replace(",", "", regex=False)
    )

    return df.to_dict("records")


def is_round_number(amount: float, minimum_amount: float) -> bool:
    """
...
def detect_round_number_anomalies(transactions: list[dict]) -> list[dict]:
    ...
    for customer_id, customer_transactions in customers.items():
        ...
        for transaction in customer_transactions:
            ...
            if is_round_number(amount=amount, minimum_amount=MINIMUM_ROUND_AMOUNT):
        ...
        if len(round_number_transactions) >= ROUND_NUMBER_FREQUENCY_THRESHOLD:
            for transaction in round_number_transactions:
                ...
                transaction["reason"] = (
                    f"Round-number transaction: {transaction['amount_kes']}"
                )
                transaction["flag"] = "round_number_anomaly"
                transaction["date_flagged"] = pandas.Timestamp.now(tz="UTC").isoformat()
...
if __name__ == "__main__":
    ...
    flagged_transactions = detect_round_number_anomalies(transactions)
